...
const API_KEY = "tinhdeptrai"; // Thay đổi key này bằng key của bạn

// Middleware for HTTP API (still keeping it for now, can be removed)
fastify.addHook("onRequest", async (request, reply) => {
  if (request.url.startsWith("/api/xoc88") || request.url.startsWith("/api/history-json")) {
...
const cauLogFilePath = path.resolve(__dirname, 'cauapisun_log.json');
...
function connectWebSocket() {
  ...
  ws.on("open", () => {
    ...
    const authPayload = [
      ...
      "tinhbip99999",
      {
        info: '{\"ipAddress\":\"2402:9d80:36a:1716:13d7:a37a:60e2:2c64\",\"userId\":\"bc260ff3-37b9-4dff-87cc-d46c102bd2d6\",\"username\":\"vantinh59777\",\"timestamp\":1752171303817,\"deviceId\":\"050105373613700053736080036024\",\"cs\":\"b6805b236a322996781a13a1e50962e4\"}',
  signature: ''
}
    ];
  ...
  ws.on("message", async (data) => {
    try {
      ...
      console.error("Lỗi khi phân tích tin nhắn WebSocket từ XOC88:", e);
    }
  });

  ws.on("close", () => {
    console.warn("Kết nối WebSocket đến Xoc88 bị đóng. Đang thử kết nối lại...");
    clearInterval(intervalCmd);
    setTimeout(connectWebSocket, reconnectInterval);
  });

  ws.on("error", (err) => {
    console.error("Lỗi WebSocket từ Xoc88:", err.message);
    ws.close();
  });
}
...
fastify.get("/api/xoc88", async (request, reply) => {
...
fastify.get("/api/xoc88/taixiu/ws", { websocket: true }, (connection, req) => {
...
const start = async () => {
  try {
    ...
    console.log(`HTTP API (for testing): http://localhost:${PORT}/api/xoc88?key=${API_KEY}`);
    console.log(`History JSON (for testing): http://localhost:${PORT}/api/history-json?key=${API_KEY}`);
    console.log(`WebSocket API (use ws:// with public IP): ws://YOUR_PUBLIC_IP:${PORT}/api/xoc88/taixiu/ws?id=vantinh597&key=${API_KEY}`);
  } catch (err) {
